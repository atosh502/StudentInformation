package com.sankalpa.jsf.beans;

import java.util.Arrays;
import java.util.List;

public class AdministrativeLevels {
	
	public static List<String> getDistricts(int province){
		if (province == 1) {
			return Arrays.asList(
					"Bhojpur District",
					"Dhankuta District",
					"Ilam District",
					"Jhapa District",
					"Khotang District",
					"Morang District",
					"Okhaldhunga District",
					"Panchthar District",
					"Sankhuwasabha district",
					"Solukhumbu District",
					"Sunsari District",
					"Taplejung District",
					"Terhathum District",
					"Udayapur District"
					);
		} else if (province == 2) {
			return Arrays.asList(
					"Saptari District",
					"Siraha District",
					"Dhanusa District",
					"Mahottari District",
					"Sarlahi District",
					"Bara District",
					"Parsa District",
					"Rautahat District"
					);
					
		} else if (province == 3) {
			return Arrays.asList(
					"Sindhuli District",
					"Ramechhap District",
					"Dolakha District",
					"Bhaktapur District",
					"Dhading District",
					"Kathmandu District",
					"Kavrepalanchok District",
					"Lalitpur District",
					"Nuwakot District",
					"Rasuwa District",
					"Sindhupalchok District",
					"Chitwan District",
					"Makwanpur District"
					);
		} else if (province == 4) {
			return Arrays.asList(
					"Gorkha District",
					"Kaski District",
					"Lamjung District",
					"Syangja District",
					"Tanahun District",
					"Manang District",
					"Nawalpur District",
					"Baglung District",
					"Myagdi District",
					"Parbat District",
					"Mustang District"
					);
			
		} else if (province == 5) {
			return Arrays.asList(
					"Kapilvastu District",
					"Parasi District",
					"Rupandehi District",
					"Arghakhanchi District",
					"Gulmi District",
					"Palpa District",
					"Dang District",
					"Pyuthan District",
					"Rolpa District",
					"Eastern Rukum District",
					"Banke District",
					"Bardiya District"
					);
			
		} else if (province == 6) {
			return Arrays.asList(
					"Western Rukum District",
					"Salyan District",
					"Dolpa District",
					"Humla District",
					"Jumla District",
					"Kalikot District",
					"Mugu District",
					"Surkhet District",
					"Dailekh District",
					"Jajarkot District"
					);	
		} else {
			return Arrays.asList(
					"Kailali District",
					"Achham District",
					"Doti District",
					"Bajhang District",
					"Bajura District",
					"Kanchanpur District",
					"Dadeldhura District",
					"Baitadi District",
					"Darchula District"
					);
		}
	}
	
	public static List<String> getLocalLevels(String district){
		
		if (district.equals("Bhojpur District")) {
			return Arrays.asList(
					"अरुण गाउँपालिका", 
					"आमचोक गाउँपालिका", 
					"ट्याम्केमैयुम गाउँपालिका", 
					"पौवादुङमा गाउँपालिका", 
					"भोजपुर नगरपालिका", 
					"रामप्रसाद राई गाउँपालिका", 
					"षडानन्द नगरपालिका", 
					"साल्पासिलिछो गाउँपालिका", 
					"हतुवागढी गाउँपालिका"
					);
		} else if (district.equals("Dhankuta District")) {
			return Arrays.asList(
					"खाल्सा छिन्ताङ सहीदभूमि गाउँपालिका",
					"चौविसे गाउँपालिका",
					"छथर जोरपाटी गाउँपालिका",
					"धनकुटा नगरपालिका",
					"पाख्रिवास नगरपालिका",
					"महालक्ष्मी नगरपालिका",
					"सागुरीगढी गाउँपालिका"
					);
		} else if (district.equals("Ilam District")) {
			return Arrays.asList(
					"इलाम नगरपालिका",
					"चुलाचुली गाउँपालिका",
					"देउमाई नगरपालिका",
					"फाकफोकथुम गाउँपालिका",
					"माई नगरपालिका",
					"माईजोगमाई गाउँपालिका"
					);
		} else if (district.equals("Jhapa District")) {
			return Arrays.asList(
					"अर्जुनधारा नगरपालिका",
					"कचनकवल गाउँपालिका",
					"कन्काई नगरपालिका",
					"कमल गाउँपालिका",
					"गौरादह नगरपालिका",
					"गौरीगंज गाउँपालिका",
					"झापा गाउँपालिका",
					"दमक नगरपालिका",
					"बाह्रदशी गाउँपालिका",
					"बुद्धशान्ति गाउँपालिका",
					"भद्रपुर नगरपालिका",
					"मेची नगर नगरपालिका",
					"विर्तामोड नगरपालिका",
					"शिवसताक्षि नगरपालिका",
					"हल्दीवारी गाउँपालिका"
					);
		} else if (district.equals("Khotang District")) {
			return Arrays.asList(
					"ऐसेलुखर्क गाउँपालिका",
					"केपिलासगढी गाउँपालिका",
					"खोटेहाङ गाउँपालिका",
					"जन्तेढुंगा गाउँपालिका",
					"दिप्रुङ गाउँपालिका",
					"बराहपोखरी गाउँपालिका",
					"रुपाकोट मजुवागढी नगरपालिका",
					"लामीडाँडा गाउँपालिका",
					"साकेला गाउँपालिका",
					"हलेसी तुवाचुङ नगरपालिका"
					);
		} else if (district.equals("Morang District")) {
			return Arrays.asList(
					"उर्लावारी नगरपालिका",
					"कटहरी गाउँपालिका",
					"कानेपोखरी गाउँपालिका",
					"केरावारी गाउँपालिका",
					"ग्रामथान गाउँपालिका",
					"जहदा गाउँपालिका",
					"धनपालथान गाउँपालिका",
					"पथरी शनिश्चरे नगरपालिका",
					"बुढीगंगा गाउँपालिका",
					"बेलवारी नगरपालिका",
					"मिक्लाजुङ गाउँपालिका",
					"रंगेली नगरपालिका",
					"रतुवामाई नगरपालिका",
					"लेटाङ नगरपालिका",
					"विराटनगर महानगरपालिका",
					"सुनवर्षी नगरपालिका",
					"सुन्दरहरैंचा नगरपालिका"
					);
		} else if (district.equals("Okhaldhunga District")) {
			return Arrays.asList(
					"खिजिदेम्वा गाउँपालिका",
					"चम्पादेवी  गाउँपालिका",
					"चिशंखुगढी गाउँपालिका",
					"मानेभञ्ज्याङ गाउँपालिका",
					"मोलुङ गाउँपालिका",
					"लिखु  गाउँपालिका",
					"सिद्दिचरण नगरपालिका",
					"सुनकोशी गाउँपालिका"
					);
		} else if (district.equals("Panchthar District")) {
			return Arrays.asList(
					"कुम्मायक गाउँपालिका",
					"तुम्वेवा गाउँपालिका",
					"फालेलुङ गाउँपालिका",
					"फाल्गुनन्द गाउँपालिका",
					"फिदिम नगरपालिका",
					"मिक्लाजुङ गाउँपालिका",
					"याङवरक गाउँपालिका",
					"हिलिहाङ गाउँपालिका"
					);
		} else if (district.equals("Sankhuwasabha district")) {
			return Arrays.asList(
					"खाँदवारी नगरपालिका",
					"चिचिला गाउँपालिका",
					"चैनपुर नगरपालिका",
					"धर्मदेवी नगरपालिका",
					"पाँचखपन नगरपालिका",
					"भोटखोला गाउँपालिका",
					"मकालु गाउँपालिका",
					"मादी नगरपालिका",
					"सभापोखरी गाउँपालिका",
					"सिलीचोङ गाउँपालिका"
					);
		} else if (district.equals("Solukhumbu District")) {
			return Arrays.asList(
					"खुम्वु पासाङल्हमु गाउँपालिका",
					"दुधकोशी गाउँपालिका",
					"दुधकौशिका गाउँपालिका",
					"नेचासल्यान गाउँपालिका",
					"महाकुलुङ गाउँपालिका",
					"लिखु पिके गाउँपालिका",
					"सोताङ गाउँपालिका",
					"सोलु दुधकुण्ड नगरपालिका"
					);
		} else if (district.equals("Sunsari District")) {
			return Arrays.asList(
					"ईटहरी उपमहानगरपालिका",
					"ईनरुवा नगरपालिका",
					"कोशी गाउँपालिका",
					"गढी गाउँपालिका",
					"दुहवी नगरपालिका",
					"देवानगन्ज गाउँपालिका",
					"धरान उपमहानगरपालिका",
					"बराह नगरपालिका",
					"बर्जु गाउँपालिका",
					"भोक्राहा गाउँपालिका",
					"रामधुनी नगरपालिका",
					"हरिनगरा गाउँपालिका"
					);
		} else if (district.equals("Taplejung District")) {
			return Arrays.asList(
					"आठराई त्रिवेणी गाउँपालिका",
					"फक्ताङलुङ गाउँपालिका",
					"फुङलिङ नगरपालिका",
					"मिक्वाखोला गाउँपालिका",
					"मेरिङदेन गाउँपालिका",
					"मैवाखोला गाउँपालिका",
					"याङवरक गाउँपालिका",
					"सिदिङ्वा गाउँपालिका",
					"सिरीजङ्घा गाउँपालिका"
					);
		} else if (district.equals("Terhathum District")) {
			return Arrays.asList(
					"आठराई  गाउँपालिका",
					"छथर गाउँपालिका",
					"फेदाप  गाउँपालिका",
					"मेन्छयायेम गाउँपालिका",
					"म्याङलुङ नगरपालिका",
					"लालिगुँरास नगरपालिका"
					);
		} else if (district.equals("Udayapur District")) {
			return Arrays.asList(
					"उदयपुरगढी गाउँपालिका",
					"कटारी  नगरपालिका",
					"चौदण्डीगढी नगरपालिका",
					"ताप्ली गाउँपालिका",
					"त्रियुगा नगरपालिका",
					"रौतामाई गाउँपालिका",
					"वेलका  नगरपालिका",
					"सुनकोशी गाउँपालिका"
					);
		} else if (district.equals("Saptari District")) {
			return Arrays.asList(
					"अग्नीसाइर कृष्णासवरन गाउँपालिका",
					"कञ्चनरुप नगरपालिका",
					"खडक नगरपालिका",
					"छिन्नमस्ता गाउँपालिका",
					"डाक्नेश्वरी नगरपालिका",
					"तिरहुत गाउँपालिका",
					"तिलाठी कोईलाडी गाउँपालिका",
					"बेल्ही चपेना गाउँपालिका",
					"बोदे बरसाइन नगरपालिका",
					"महादेवा गाउँपालिका",
					"राजविराज नगरपालिका",
					"रुपनी गाउँपालिका",
					"वलान-विहुल गाउँपालिका",
					"विष्णुपुर गाउँपालिका",
					"शम्भुनाथ  नगरपालिका",
					"सप्तकोशी नगरपालिका",
					"सुरुङ्गा नगरपालिका",
					"हनुमाननगर कङ्कालिनी नगरपालिका"
					);
		} else if (district.equals("Siraha District")) {
			return Arrays.asList(
					"अर्नमा गाउँपालिका",
					"औरही गाउँपालिका",
					"कर्जन्हा नगरपालिका",
					"कल्याणपुर नगरपालिका",
					"गोलबजार नगरपालिका",
					"धनगढीमाई नगरपालिका",
					"नरहा गाउँपालिका",
					"नवराजपुर गाउँपालिका",
					"बरियारपट्टी गाउँपालिका",
					"भगवानपुर गाउँपालिका",
					"मिर्चैया नगरपालिका",
					"लक्ष्मीपुर पतारी गाउँपालिका",
					"लहान नगरपालिका",
					"विष्णुपुर गाउँपालिका",
					"सखुवानान्कारकट्टी गाउँपालिका",
					"सिराहा नगरपालिका",
					"सुखीपुर नगरपालिका"
					);
		} else if (district.equals("Dhanusa District")) {
			return Arrays.asList(
					"औरही  गाउँपालिका",
					"कमला नगरपालिका",
					"क्षिरेश्वरनाथ नगरपालिका",
					"गणेशमान चारनाथ नगरपालिका",
					"जनकनन्दिनी गाउँपालिका",
					"जनकपुर उपमहानगरपालिका",
					"धनुषाधाम नगरपालिका",
					"धनौजी गाउँपालिका",
					"नगराइन नगरपालिका",
					"बटेश्वर गाउँपालिका",
					"मिथिला नगरपालिका",
					"मिथिला बिहारी नगरपालिका",
					"मुखियापट्टी मुसहरमिया गाउँपालिका",
					"लक्ष्मीनिया गाउँपालिका",
					"विदेह नगरपालिका",
					"शहीदनगर नगरपालिका",
					"सबैला नगरपालिका",
					"हंसपुर नगरपालिका"
					);
		} else if (district.equals("Mahottari District")) {
			return Arrays.asList(
					"एकडारा गाउँपालिका",
					"औरही नगरपालिका",
					"गौशाला नगरपालिका",
					"जलेश्वर नगरपालिका",
					"पिपरा गाउँपालिका",
					"बर्दिबास नगरपालिका",
					"बलवा नगरपालिका",
					"भँगाहा नगरपालिका",
					"मटिहानी नगरपालिका",
					"मनरा शिसवा नगरपालिका",
					"महोत्तरी गाउँपालिका",
					"रामगोपालपुर नगरपालिका",
					"लोहरपट्टी नगरपालिका",
					"साम्सी गाउँपालिका",
					"सोनमा गाउँपालिका"
					);
		} else if (district.equals("Sarlahi District")) {
			return Arrays.asList(
					"ईश्वरपुर नगरपालिका",
					"कविलासी नगरपालिका",
					"कौडेना गाउँपालिका",
					"गोडैटा  नगरपालिका",
					"चक्रघट्टा  गाउँपालिका",
					"चन्द्रनगर गाउँपालिका",
					"धनकौल गाउँपालिका",
					"पर्सा गाउँपालिका",
					"बरहथवा नगरपालिका",
					"बलरा नगरपालिका",
					"बसबरिया गाउँपालिका",
					"बागमती नगरपालिका",
					"ब्रह्मपुरी  गाउँपालिका",
					"मलंगवा नगरपालिका",
					"रामनगर गाउँपालिका",
					"लालबन्दी नगरपालिका",
					"विष्णु गाउँपालिका",
					"हरिपुर नगरपालिका",
					"हरिपुर्वा नगरपालिका",
					"हरिवन नगरपालिका"
					);
		} else if (district.equals("Bara District")) {
			return Arrays.asList(
					"आदर्श कोटवाल गाउँपालिका",
					"करैयामाई गाउँपालिका",
					"कलैया उपमहानगरपालिका",
					"कोल्हवी नगरपालिका",
					"जीतपुरसिमरा उपमहानगरपालिका",
					"देवताल गाउँपालिका",
					"निजगढ नगरपालिका",
					"पचरौता नगरपालिका",
					"परवानीपुर गाउँपालिका",
					"प्रसौनी गाउँपालिका",
					"फेटा गाउँपालिका",
					"बारागढी गाउँपालिका",
					"महागढीमाई नगरपालिका",
					"विश्रामपुर गाउँपालिका",
					"सिम्रौनगढ नगरपालिका",
					"सुवर्ण गाउँपालिका"
					);
		} else if (district.equals("Parsa District")) {
			return Arrays.asList(
					"कालिकामाई गाउँपालिका",
					"छिपहरमाई गाउँपालिका",
					"जगरनाथपुर गाउँपालिका",
					"जिराभवानी गाउँपालिका",
					"ठोरी गाउँपालिका",
					"धोबीनी गाउँपालिका",
					"पकाहा मैनपुर गाउँपालिका",
					"पटेर्वा सुगौली गाउँपालिका",
					"पर्सागढी नगरपालिका",
					"पोखरिया नगरपालिका",
					"बहुदरमाई नगरपालिका",
					"बिन्दबासिनी गाउँपालिका",
					"बिरगंज महानगरपालिका",
					"सखुवा प्रसौनी गाउँपालिका"
					);
		} else if (district.equals("Rautahat District")) {
			return Arrays.asList(
					"ईशनाथ नगरपालिका",
					"कटहरिया नगरपालिका",
					"गढीमाई नगरपालिका",
					"गरुडा नगरपालिका",
					"गुजरा नगरपालिका",
					"गौर नगरपालिका",
					"चन्द्रपुर नगरपालिका",
					"दुर्गा भगवती गाउँपालिका",
					"देवाही गोनाही   नगरपालिका",
					"परोहा नगरपालिका",
					"फतुवा बिजयपुर नगरपालिका",
					"बौधीमाई नगरपालिका",
					"माधव नारायण नगरपालिका",
					"मौलापुर नगरपालिका",
					"यमुनामाई  गाउँपालिका",
					"राजदेवी नगरपालिका",
					"राजपुर नगरपालिका",
					"वृन्दावन नगरपालिका"
					);
		} else if (district.equals("Sindhuli District")) {
			return Arrays.asList(
					"कमलामाई नगरपालिका",
					"गोलन्जोर गाउँपालिका",
					"घ्याङलेख गाउँपालिका",
					"तिनपाटन गाउँपालिका",
					"दुधौली नगरपालिका",
					"फिक्कल गाउँपालिका",
					"मरिण गाउँपालिका",
					"सुनकोशी गाउँपालिका",
					"हरिहरपुरगढी गाउँपालिका"
					);
		} else if (district.equals("Ramechhap District")) {
			return Arrays.asList(
					"उमाकुण्ड गाउँपालिका",
					"खाँडादेवी गाउँपालिका",
					"गोकुलगङ्गा गाउँपालिका",
					"दोरम्बा गाउँपालिका",
					"मन्थली नगरपालिका",
					"रामेछाप नगरपालिका",
					"लिखु गाउँपालिका",
					"सुनापती गाउँपालिका"
					);
		} else if (district.equals("Dolakha District")) {
			return Arrays.asList(
					"कालिन्चोक  गाउँपालिका",
					"गौरिशंकर  गाउँपालिका",
					"जिरी  नगरपालिका",
					"तामाकोशी  गाउँपालिका",
					"भिमेश्वर  नगरपालिका",
					"मेलुङ  गाउँपालिका",
					"विगु गाउँपालिका",
					"वैतेश्वर  गाउँपालिका",
					"शैलुङ  गाउँपालिका"
					);
		} else if (district.equals("Bhaktapur District")) {
			return Arrays.asList(
					"चाँगुनारायण नगरपालिका",
					"भक्तपुर नगरपालिका",
					"मध्यपुर थिमी नगरपालिका",
					"सूर्यविनायक नगरपालिका"
					);
		} else if (district.equals("Dhading District")) {
			return Arrays.asList(
					"खनियाबास गाउँपालिका",
					"गङ्गाजमुना गाउँपालिका",
					"गजुरी गाउँपालिका",
					"गल्छी गाउँपालिका",
					"ज्वालामूखी गाउँपालिका",
					"त्रिपुरासुन्दरी गाउँपालिका",
					"थाक्रे गाउँपालिका",
					"धुनीबेंशी नगरपालिका",
					"निलकण्ठ नगरपालिका",
					"नेत्रावती गाउँपालिका",
					"बेनीघाट रोराङ्ग गाउँपालिका",
					"रुवी भ्याली गाउँपालिका",
					"सिद्धलेक गाउँपालिका"
					);
		} else if (district.equals("Kathmandu District")) {
			return Arrays.asList(
					"कागेश्वरी मनोहरा नगरपालिका",
					"काठमाण्डौ महानगरपालिका",
					"किर्तिपुर नगरपालिका",
					"गोकर्णेश्वर नगरपालिका",
					"चन्द्रागिरी नगरपालिका",
					"टोखा नगरपालिका",
					"तारकेश्वर नगरपालिका",
					"दक्षिणकाली नगरपालिका",
					"नागार्जुन नगरपालिका",
					"बुढानिलकण्ठ नगरपालिका",
					"शङ्खरापुर नगरपालिका"
					);
		} else if (district.equals("Kavrepalanchok District")) {
			return Arrays.asList(
					"खानीखोला गाउँपालिका",
					"चौंरीदेउराली गाउँपालिका",
					"तेमाल गाउँपालिका",
					"धुलिखेल नगरपालिका",
					"नमोबुद्ध नगरपालिका",
					"पनौती नगरपालिका",
					"पाँचखाल नगरपालिका",
					"बनेपा नगरपालिका",
					"बेथानचोक गाउँपालिका",
					"भुम्लु गाउँपालिका",
					"मण्डनदेउपुर नगरपालिका",
					"महाभारत गाउँपालिका",
					"रोशी गाउँपालिका"
					);
		} else if (district.equals("Lalitpur District")) {
			return Arrays.asList(
					"कोन्ज्योसोम गाउँपालिका",
					"गोदावरी नगरपालिका",
					"बाग्मती गाउँपालिका",
					"महाङ्काल गाउँपालिका",
					"महालक्ष्मी नगरपालिका",
					"ललितपुर महानगरपालिका"
					);
		} else if (district.equals("Nuwakot District")) {
			return Arrays.asList(
					"ककनी गाउँपालिका",
					"किस्पाङ गाउँपालिका",
					"तादी गाउँपालिका",
					"तारकेश्वर गाउँपालिका",
					"दुप्चेश्वर गाउँपालिका",
					"पञ्चकन्या गाउँपालिका",
					"बेलकोटगढी नगरपालिका",
					"मेघाङ गाउँपालिका",
					"लिखु गाउँपालिका",
					"विदुर नगरपालिका",
					"शिवपुरी गाउँपालिका",
					"सुर्यगढी गाउँपालिका"
					);
		} else if (district.equals("Rasuwa District")) {
			return Arrays.asList(
					"उत्तरगया गाउँपालिका",
					"कालिका गाउँपालिका",
					"गोसाईकुण्ड गाउँपालिका",
					"नौकुण्ड गाउँपालिका",
					"पार्वतीकुण्ड गाउँपालिका"
					);
		} else if (district.equals("Sindhupalchok District")) {
			return Arrays.asList(
					"चौतारा साँगाचोकगढी नगरपालिका",
					"जुगल गाउँपालिका",
					"त्रिपुरासुन्दरी गाउँपालिका",
					"पाँचपोखरी थाङपाल गाउँपालिका",
					"बलेफी गाउँपालिका",
					"भोटेकोशी गाउँपालिका",
					"मेलम्ची नगरपालिका",
					"र्इन्द्रावती गाउँपालिका",
					"लिसंखु पाखर गाउँपालिका",
					"वाह्रविसे नगरपालिका",
					"सुनकोशी गाउँपालिका",
					"हेलम्बु  गाउँपालिका"
					);
		} else if (district.equals("Chitwan District")) {
			return Arrays.asList(
					"इच्छाकामना गाउँपालिका",
					"कालिका नगरपालिका",
					"खैरहनी नगरपालिका",
					"भरतपुर महानगरपालिका",
					"माडी नगरपालिका",
					"रत्ननगर नगरपालिका",
					"राप्ती नगरपालिका"
					);
		} else if (district.equals("Makwanpur District")) {
			return Arrays.asList(
					"ईन्द्र सरोवर गाउँपालिका",
					"कैलाश गाउँपालिका",
					"थाहा नगरपालिका",
					"बकैया गाउँपालिका",
					"बाग्मती गाउँपालिका",
					"भीमफेदी गाउँपालिका",
					"मकवानपुरगढी गाउँपालिका",
					"मनहरी गाउँपालिका",
					"राक्सिराङ्ग गाउँपालिका",
					"हेटौडा उपमहानगरपालिका"
					);
		} else if (district.equals("Gorkha District")) {
			return Arrays.asList(
					"अजिरकोट गाउँपालिका",
					"आरूघाट गाउँपालिका",
					"गण्डकी गाउँपालिका",
					"गोरखा नगरपालिका",
					"चुम नुव्री गाउँपालिका",
					"धार्चे गाउँपालिका",
					"पालुङटार नगरपालिका",
					"भिमसेन गाउँपालिका",
					"शहिद लखन गाउँपालिका",
					"सिरानचोक गाउँपालिका",
					"सुलीकोट गाउँपालिका"
					);
		} else if (district.equals("Kaski District")) {
			return Arrays.asList(
					"अन्नपुर्ण गाउँपालिका",
					"पोखरा लेखनाथ महानगरपालिका",
					"माछापुछ्रे गाउँपालिका",
					"मादी गाउँपालिका",
					"रूपा गाउँपालिका"
					);
		} else if (district.equals("Lamjung District")) {
			return Arrays.asList(
					"क्व्होलासोथार गाउँपालिका",
					"दूधपोखरी गाउँपालिका",
					"दोर्दी गाउँपालिका",
					"बेसीशहर नगरपालिका",
					"मध्यनेपाल नगरपालिका",
					"मर्स्याङदी गाउँपालिका",
					"रार्इनास नगरपालिका",
					"सुन्दरबजार नगरपालिका"
					);
		} else if (district.equals("Syangja District")) {
			return Arrays.asList(
					"अर्जुन चौपारी गाउँपालिका",
					"आँधीखोला गाउँपालिका",
					"कालीगण्डकी गाउँपालिका",
					"गल्याङ नगरपालिका",
					"चापाकोट नगरपालिका",
					"पुतलीबजार नगरपालिका",
					"फेदीखोला गाउँपालिका",
					"भिरकोट नगरपालिका",
					"वालिङ नगरपालिका",
					"विरुवा गाउँपालिका",
					"हरीनास गाउँपालिका"
					);
		} else if (district.equals("Tanahun District")) {
			return Arrays.asList(
					"आँबुखैरेनी गाउँपालिका",
					"ऋषिङ्ग गाउँपालिका",
					"घिरिङ गाउँपालिका",
					"देवघाट गाउँपालिका",
					"बन्दिपुर गाउँपालिका",
					"भानु नगरपालिका",
					"भिमाद नगरपालिका",
					"म्याग्दे गाउँपालिका",
					"व्यास नगरपालिका",
					"शुक्लागण्डकी नगरपालिका"
					);
		} else if (district.equals("Manang District")) {
			return Arrays.asList(
					"चामे गाउँपालिका",
					"नारफू गाउँपालिका",
					"नाशोङ गाउँपालिका",
					"नेस्याङ गाउँपालिका"
					);
		} else if (district.equals("Nawalpur District")) {
			return Arrays.asList(
					"कावासोती नगरपालिका",
					"गैडाकोट नगरपालिका",
					"देवचुली नगरपालिका",
					"बुङदीकाली गाउँपालिका",
					"बुलिङटार गाउँपालिका",
					"मध्यविन्दु नगरपालिका",
					"विनयी त्रिवेणी गाउँपालिका",
					"हुप्सेकोट गाउँपालिका"
					);
		} else if (district.equals("Baglung District")) {
			return Arrays.asList(
					"काठेखोला गाउँपालिका",
					"गल्कोट नगरपालिका",
					"जैमुनी नगरपालिका",
					"ढोरपाटन नगरपालिका",
					"तमानखोला गाउँपालिका",
					"ताराखोला गाउँपालिका",
					"निसीखोला गाउँपालिका",
					"बागलुङ नगरपालिका",
					"वडिगाड गाउँपालिका",
					"वरेङ गाउँपालिका"
					);
		} else if (district.equals("Myagdi District")) {
			return Arrays.asList(
					"अन्नपूर्ण गाउँपालिका",
					"धवलागिरी गाउँपालिका",
					"बेनी  नगरपालिका",
					"मंगला गाउँपालिका",
					"मालिका गाउँपालिका",
					"रघुगंगा गाउँपालिका"
					);
		} else if (district.equals("Parbat District")) {
			return Arrays.asList(
					"कुश्मा नगरपालिका",
					"जलजला गाउँपालिका",
					"पैयूं गाउँपालिका",
					"फलेवास नगरपालिका",
					"महाशिला गाउँपालिका",
					"मोदी गाउँपालिका",
					"विहादी गाउँपालिका"
					);
		} else if (district.equals("Mustang District")) {
			return Arrays.asList(
					"घरपझोङ गाउँपालिका",
					"थासाङ गाउँपालिका",
					"दालोमे गाउँपालिका",
					"बाह्गाउँ मुक्तिक्षेत्र गाउँपालिका",
					"लोमन्थाङ गाउँपालिका"
					);
		} else if (district.equals("Kapilvastu District")) {
			return Arrays.asList(
					"कपिलवस्तु नगरपालिका",
					"कृष्णनगर नगरपालिका",
					"बाणगंगा नगरपालिका",
					"बुद्धभूमी नगरपालिका",
					"महाराजगंज नगरपालिका",
					"मायादेवी गाउँपालिका",
					"यसोधरा गाउँपालिका",
					"विजयनगर गाउँपालिका",
					"शिवराज नगरपालिका",
					"शुद्धोधन गाउँपालिका"
					);
		} else if (district.equals("Parasi District")) {
			return Arrays.asList(
					"पाल्हीनन्दन गाउँपालिका",
					"प्रतापपुर गाउँपालिका",
					"बर्दघाट नगरपालिका",
					"रामग्राम नगरपालिका",
					"सरावल गाउँपालिका",
					"सुनवल नगरपालिका",
					"सुस्ता गाउँपालिका"
					);
		} else if (district.equals("Rupandehi District")) {
			return Arrays.asList(
					"ओमसतिया गाउँपालिका",
					"कंचन गाउँपालिका",
					"कोटहीमाई गाउँपालिका",
					"गैडहवा गाउँपालिका",
					"तिलोत्तमा नगरपालिका",
					"देवदह नगरपालिका",
					"बुटवल उपमहानगरपालिका",
					"मर्चवारी गाउँपालिका",
					"मायादेवी गाउँपालिका",
					"रोहिणी गाउँपालिका",
					"लुम्बिनी सांस्कृतिक नगरपालिका",
					"शुद्धोधन गाउँपालिका",
					"सम्मरीमाई गाउँपालिका",
					"सिद्धार्थनगर नगरपालिका",
					"सियारी गाउँपालिका",
					"सैनामैना नगरपालिका"
					);
		} else if (district.equals("Arghakhanchi District")) {
			return Arrays.asList(
					"छत्रदेव गाउँपालिका",
					"पाणिनी गाउँपालिका",
					"भूमिकास्थान नगरपालिका",
					"मालारानी गाउँपालिका",
					"सन्धिखर्क नगरपालिका",
					"सितगंगा नगरपालिका"
					);
		} else if (district.equals("Gulmi District")) {
			return Arrays.asList(
					"ईस्मा गाउँपालिका",
					"कालीगण्डकी गाउँपालिका",
					"गुल्मीदरवार गाउँपालिका",
					"चन्द्रकोट गाउँपालिका",
					"छत्रकोट गाउँपालिका",
					"धुर्कोट गाउँपालिका",
					"मदाने गाउँपालिका",
					"मालिका गाउँपालिका",
					"मुसिकोट नगरपालिका",
					"रुरु गाउँपालिका",
					"रेसुङ्गा नगरपालिका",
					"सत्यवती गाउँपालिका"
					);
		} else if (district.equals("Palpa District")) {
			return Arrays.asList(
					"तानसेन नगरपालिका",
					"तिनाउ गाउँपालिका",
					"निस्दी गाउँपालिका",
					"पूर्वखोला गाउँपालिका",
					"बगनासकाली गाउँपालिका",
					"माथागढी गाउँपालिका",
					"रम्भा गाउँपालिका",
					"रामपुर नगरपालिका",
					"रिब्दिकोट गाउँपालिका",
					"रैनादेवी छहरा गाउँपालिका"
					);
		} else if (district.equals("Dang District")) {
			return Arrays.asList(
					"गढवा गाउँपालिका",
					"घोराही उपमहानगरपालिका",
					"तुल्सिपुर उपमहानगरपालिका",
					"दंगीशरण गाउँपालिका",
					"बंगलाचुली गाउँपालिका",
					"बबई गाउँपालिका",
					"राजपुर गाउँपालिका",
					"राप्ती गाउँपालिका",
					"लमही नगरपालिका",
					"शान्तिनगर गाउँपालिका"
					);
		} else if (district.equals("Pyuthan District")) {
			return Arrays.asList(
					"ऐरावती गाउँपालिका",
					"गौमुखी गाउँपालिका",
					"झिमरुक गाउँपालिका",
					"नौवहिनी गाउँपालिका",
					"प्यूठान नगरपालिका",
					"मल्लरानी गाउँपालिका",
					"माण्डवी गाउँपालिका",
					"सरुमारानी गाउँपालिका",
					"स्वर्गद्वारी नगरपालिका"
					);
		} else if (district.equals("Rolpa District")) {
			return Arrays.asList(
					"त्रिवेणी गाउँपालिका",
					"थवाङ गाउँपालिका",
					"दुईखोली गाउँपालिका",
					"माडी गाउँपालिका",
					"रुन्टीगढी गाउँपालिका",
					"रोल्पा नगरपालिका",
					"लुङग्री गाउँपालिका",
					"सुकिदह गाउँपालिका",
					"सुनछहरी गाउँपालिका",
					"सुवर्णावती गाउँपालिका"
					);
		} else if (district.equals("Eastern Rukum District")) {
			return Arrays.asList(
					"पुथा उत्तरगंगा गाउँपालिका",
					"भूमे गाउँपालिका",
					"सिस्ने गाउँपालिका"
					);
		} else if (district.equals("Banke District")) {
			return Arrays.asList(
					"कोहलपुर नगरपालिका",
					"खजुरा गाउँपालिका",
					"जानकी गाउँपालिका",
					"डुडुवा गाउँपालिका",
					"नरैनापुर गाउँपालिका",
					"नेपालगंज उपमहानगरपालिका",
					"बैजनाथ गाउँपालिका",
					"राप्ती सोनारी गाउँपालिका"
					);
		} else if (district.equals("Bardiya District")) {
			return Arrays.asList(
					"गुलरिया नगरपालिका",
					"गेरुवा गाउँपालिका",
					"ठाकुरबाबा नगरपालिका",
					"बढैयाताल गाउँपालिका",
					"बारबर्दिया नगरपालिका",
					"बासगढी नगरपालिका",
					"मधुवन नगरपालिका",
					"राजापुर नगरपालिका"
					);
		} else if (district.equals("Western Rukum District")) {
			return Arrays.asList(
					"आठबिसकोट नगरपालिका",
					"चौरजहारी नगरपालिका",
					"त्रिवेणी गाउँपालिका",
					"बाँफिकोट गाउँपालिका",
					"मुसिकोट नगरपालिका",
					"सानीभेरी गाउँपालिका"
					);
		} else if (district.equals("Salyan District")) {
			return Arrays.asList(
					"कपुरकोट गाउँपालिका",
					"कालीमाटी गाउँपालिका",
					"कुमाखमालिका गाउँपालिका",
					"छत्रेश्वरी गाउँपालिका",
					"ढोरचौर गाउँपालिका",
					"त्रिवेणी गाउँपालिका",
					"दार्मा गाउँपालिका",
					"बनगाड कुपिण्डे नगरपालिका",
					"बागचौर नगरपालिका",
					"शारदा नगरपालिका"
					);
		} else if (district.equals("Dolpa District")) {
			return Arrays.asList(
					"काईके गाउँपालिका",
					"छार्का ताङसोङ गाउँपालिका",
					"जगदुल्ला गाउँपालिका",
					"ठूली भेरी नगरपालिका",
					"डोल्पो बुद्ध गाउँपालिका",
					"त्रिपुरासुन्दरी नगरपालिका",
					"मुड्केचुला गाउँपालिका",
					"शे फोक्सुन्डो गाउँपालिका"
					);
		} else if (district.equals("Humla District")) {
			return Arrays.asList(
					"अदानचुली गाउँपालिका",
					"खार्पुनाथ गाउँपालिका",
					"चंखेली गाउँपालिका",
					"ताँजाकोट गाउँपालिका",
					"नाम्खा गाउँपालिका",
					"सर्केगाड गाउँपालिका",
					"सिमकोट गाउँपालिका"
					);
		} else if (district.equals("Jumla District")) {
			return Arrays.asList(
					"कनकासुन्दरी गाउँपालिका",
					"गुठिचौर गाउँपालिका",
					"चन्दननाथ नगरपालिका",
					"तातोपानी गाउँपालिका",
					"तिला गाउँपालिका",
					"पातारासी गाउँपालिका",
					"सिंजा गाउँपालिका",
					"हिमा गाउँपालिका"
					);
		} else if (district.equals("Kalikot District")) {
			return Arrays.asList(
					"कालिका गाउँपालिका",
					"खाँडाचक्र नगरपालिका",
					"तिलागुफा नगरपालिका",
					"नरहरिनाथ गाउँपालिका",
					"पचालझरना गाउँपालिका",
					"पलाता गाउँपालिका",
					"महावै गाउँपालिका",
					"रास्कोट नगरपालिका",
					"सान्नी त्रिवेणी गाउँपालिका"
					);
		} else if (district.equals("Mugu District")) {
			return Arrays.asList(
					"खत्याड गाउँपालिका",
					"छायाँनाथ रारा नगरपालिका",
					"मुगुम कार्मारोंग गाउँपालिका",
					"सोरु गाउँपालिका"
					);
		} else if (district.equals("Surkhet District")) {
			return Arrays.asList(
					"गुर्भाकोट नगरपालिका",
					"चिङ्गाड गाउँपालिका",
					"चौकुने गाउँपालिका",
					"पञ्चपुरी नगरपालिका",
					"बराहताल गाउँपालिका",
					"बीरेन्द्रनगर नगरपालिका",
					"भेरीगंगा नगरपालिका",
					"लेकबेशी नगरपालिका",
					"सिम्ता गाउँपालिका"
					);
		} else if (district.equals("Dailekh District")) {
			return Arrays.asList(
					"आठबीस नगरपालिका",
					"गुराँस गाउँपालिका",
					"चामुण्डा विन्द्रासैनी नगरपालिका",
					"ठाँटीकाँध गाउँपालिका",
					"डुंगेश्वर गाउँपालिका",
					"दुल्लु नगरपालिका",
					"नारायण नगरपालिका",
					"नौमुले गाउँपालिका",
					"भगवतीमाई गाउँपालिका",
					"भैरवी गाउँपालिका",
					"महावु गाउँपालिका"
					);
		} else if (district.equals("Jajarkot District")) {
			return Arrays.asList(
					"कुसे गाउँपालिका",
					"छेडागाड नगरपालिका",
					"जुनीचाँदे गाउँपालिका",
					"त्रिवेणी नलगाड नगरपालिका",
					"बारेकोट गाउँपालिका",
					"भेरी नगरपालिका",
					"शिवालय गाउँपालिका"
					);
		} else if (district.equals("Kailali District")) {
			return Arrays.asList(
					"कैलारी गाउँपालिका",
					"गोदावरी नगरपालिका",
					"गौरीगंगा नगरपालिका",
					"घोडाघोडी नगरपालिका",
					"चुरे गाउँपालिका",
					"जानकी गाउँपालिका",
					"जोशीपुर गाउँपालिका",
					"टिकापुर नगरपालिका",
					"धनगढी उपमहानगरपालिका",
					"बर्दगोरीया गाउँपालिका",
					"भजनी नगरपालिका",
					"मोहन्याल गाउँपालिका",
					"लम्की चुहा नगरपालिका"
					);
		} else if (district.equals("Achham District")) {
			return Arrays.asList(
					"कमलबजार नगरपालिका",
					"चौरपाटी गाउँपालिका",
					"ढकारी गाउँपालिका",
					"तुर्माखाँद गाउँपालिका",
					"पञ्चदेवल विनायक नगरपालिका",
					"बान्नीगढी जयगढ गाउँपालिका",
					"मंगलसेन नगरपालिका",
					"मेल्लेख गाउँपालिका",
					"रामारोशन गाउँपालिका",
					"साफेबगर नगरपालिका"
					);
		} else if (district.equals("Doti District")) {
			return Arrays.asList(
					"आदर्श गाउँपालिका",
					"के.आई.सिं. गाउँपालिका",
					"जोरायल गाउँपालिका",
					"दिपायल सिलगढी नगरपालिका",
					"पूर्वीचौकी गाउँपालिका",
					"बडीकेदार गाउँपालिका",
					"बोगटान गाउँपालिका",
					"शिखर नगरपालिका",
					"सायल गाउँपालिका"
					);
		} else if (district.equals("Bajhang District")) {
			return Arrays.asList(
					"काँडा गाउँपालिका",
					"केदारस्युँ गाउँपालिका",
					"खप्तडछान्ना गाउँपालिका",
					"छबिसपाथिभेरा गाउँपालिका",
					"जयपृथ्वी नगरपालिका",
					"तलकोट गाउँपालिका",
					"थलारा गाउँपालिका",
					"दुर्गाथली गाउँपालिका",
					"बुंगल नगरपालिका",
					"मष्टा गाउँपालिका",
					"वित्थडचिर गाउँपालिका",
					"सूर्मा गाउँपालिका"
					);
		} else if (district.equals("Bajura District")) {
			return Arrays.asList(
					"गौमुल गाउँपालिका",
					"छेडेदह गाउँपालिका",
					"त्रिवेणी नगरपालिका",
					"पाण्डवगुफा गाउँपालिका",
					"बडीमालिका नगरपालिका",
					"बुढीगंगा नगरपालिका",
					"बुढीनन्दा नगरपालिका",
					"स्वामीकार्तिक गाउँपालिका",
					"हिमाली गाउँपालिका"
					);
		} else if (district.equals("Kanchanpur District")) {
			return Arrays.asList(
					"कृष्णपुर नगरपालिका",
					"पुनर्वास नगरपालिका",
					"बेलडाडी गाउँपालिका",
					"बेलौरी नगरपालिका",
					"भीमदत्त नगरपालिका",
					"माहाकाली नगरपालिका",
					"लालझाँडी गाउँपालिका",
					"वेदकोट नगरपालिका",
					"सुक्लाफाँटा नगरपालिका"
					);
		} else if (district.equals("Dadeldhura District")) {
			return Arrays.asList(
					"अजयमेरु गाउँपालिका",
					"अमरगढी नगरपालिका",
					"आलिताल गाउँपालिका",
					"गन्यापधुरा गाउँपालिका",
					"नवदुर्गा गाउँपालिका",
					"परशुराम नगरपालिका",
					"भागेश्वर गाउँपालिका"
					);
		} else if (district.equals("Baitadi District")) {
			return Arrays.asList(
					"डीलासैनी गाउँपालिका",
					"दशरथचन्द नगरपालिका",
					"दोगडाकेदार गाउँपालिका",
					"पन्चेश्वर गाउँपालिका",
					"पाटन नगरपालिका",
					"पुर्चौडी नगरपालिका",
					"मेलौली नगरपालिका",
					"शिवनाथ गाउँपालिका",
					"सिगास गाउँपालिका",
					"सुर्नया गाउँपालिका"
					);
		} else {
			return Arrays.asList(
					"अपिहिमाल गाउँपालिका",
					"दुहुँ गाउँपालिका",
					"नौगाड गाउँपालिका",
					"ब्यास गाउँपालिका",
					"महाकाली नगरपालिका",
					"मार्मा गाउँपालिका",
					"मालिकार्जुन गाउँपालिका",
					"लेकम गाउँपालिका",
					"शैल्यशिखर नगरपालिका"
					);
		}
	
	}

}
